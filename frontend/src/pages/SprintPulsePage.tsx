...
import {
  ...
  LayoutGrid, AlignLeft, BarChart2, Layers, Tag, Activity,
...
// Compute the actual wall-clock time of a transition using hours_in_state (timezone-safe)
...
// Hover content for activity feed rows — shows transition path + rich detail NOT in the row
...
function LiveDevCard({
  ...
  // Find matching PulseIssue for the current active ticket (has hours_in_state)
    ...
    <div className={`slv-dev-card slv-dev-card--${status}`}>
      ...
      {currentIss && (
        <div className="slv-dev-current">
          ...
          <HoverCard content={pulseIss ? buildHoverContent(pulseIss) : <div style={{ fontSize: 12 }}>{currentIss.summary}</div>} delay={250}>
            <span
              className="spl-ticket-id"
              style={{ fontSize: 11 }}
...
function LiveActivityFeed({
  ...
  const { sorted, todayOnly } = useMemo(() => {
    // Sort by hours_in_state ascending (smallest = most recently moved)
...
function LiveAttention({
  ...
  function AttentionRow({ iss, idx = 0 }: { iss: PulseIssue; idx?: number }) {
    return (
      <motion.div
    ...
    <div className="slv-attn-sections">
      {blocked.length > 0 && (
        <div className="slv-attn-section slv-attn-section--blocked">
          ...
          {blocked.map((i, idx) => <AttentionRow key={i.id} iss={i} idx={idx} />)}
        </div>
      )}
      {hotfixes.length > 0 && (
        <div className="slv-attn-section slv-attn-section--hf">
          ...
          {hotfixes.map((i, idx) => <AttentionRow key={i.id} iss={i} idx={idx} />)}
        </div>
      )}
      {stuck.length > 0 && (
        <div className="slv-attn-section slv-attn-section--stuck">
          ...
          {stuck.map((i, idx) => <AttentionRow key={i.id} iss={i} idx={idx} />)}
        </div>
      )}
      {carryRisk.length > 0 && (
        <div className="slv-attn-section slv-attn-section--carry">
          ...
          {carryRisk.map((i, idx) => <AttentionRow key={i.id} iss={i} idx={idx} />)}
        </div>
      )}
    </div>
  )
...
function ViewLive({
  ...
  return (
    <motion.div className="slv-root" variants={slvStagger} initial="hidden" animate="show">
      {/* Sprint status bar */}
      {activeSprint && summary && (
        <motion.div className="slv-status-bar" variants={slvFadeUp}>
      ...
      {/* Counter chips */}
      <motion.div className="slv-counters" variants={slvFadeUp}>
        {([
        ...
        <AnimatePresence>
          {hotfixCount > 0 && (
            <motion.div
              className="slv-counter slv-counter--danger"
      ...
      {/* 3-column layout */}
      <motion.div className="slv-grid" variants={slvFadeUp}>
        {/* Left — Developer status */}
...
const SK_W = [55, 70, 48, 65, 58, 72] // vary name bar widths
